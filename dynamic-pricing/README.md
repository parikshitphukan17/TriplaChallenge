...
### 2. Proactive Bulk Pre-fetching via Containerized Scheduler
* **Why it beats Cache-Aside:**
  ...
  - **Our Solution:** A decoupled background `scheduler` container triggers the Rake task `rates:refresh` every **4 minutes** to run the bulk pre-fetch. This completely removes the fragile, in-process background threads from Puma server processes, saving memory and avoiding thread lifecycle issues during restarts. 
  - **Avoiding Rails Initializers:** Running scheduler loops from initializers (`config/initializers/`) is an anti-pattern. Because initializers run on every Puma worker process (fork), running 4 workers would spawn 4 duplicate threads, multiplying API calls and exhausting the quota. Initializers also run during normal CLI commands (like migrations, console, and testing), which would trigger unwanted API fetches.
